...
async function startServer() {
  ...
  let groups: Record<string, GroupChat> = {};
  ...
  const loadData = async () => {
    ...
    try {
      const gData = await fs.readFile(GROUPS_FILE, "utf-8");
      groups = JSON.parse(gData);
    } catch (e) {}
    ...
    // Ensure Devs exist and are friended
    DEVS.forEach(devUsername => {
      if (!users[devUsername]) {
        users[devUsername] = {
          username: devUsername,
          avatarUrl: `https://picsum.photos/seed/${devUsername}/200/200`,
          password: devUsername,
          description: `A developer for Fazbear's Tech Academy. Responsible for system core architecture.`,
          maxUnlockedNight: 7,
          highScores: { Easy: 0, Medium: 0, Hard: 0 }
        };
      } else {
        // Force update dev password to match username as requested
        users[devUsername].password = devUsername;
      }
    });

    // Ensure all devs are friends with each other
    DEVS.forEach(dev1 => {
      DEVS.forEach(dev2 => {
        if (dev1 !== dev2) {
          const exists = requests.find(r => 
            (r.from === dev1 && r.to === dev2) ||
            (r.from === dev2 && r.to === dev1)
          );
          if (!exists) {
            requests.push({ from: dev1, to: dev2, status: 'accepted' });
    ...
    await saveData();
    console.log("Loaded system data. Users:", Object.keys(users).length);
  };
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("create_group_chat", async ({ name, members, creator }: { name: string, members: string[], creator: string }) => {
      const id = Math.random().toString(36).substr(2, 9);
      const allMembers = Array.from(new Set([...members, creator]));
      groups[id] = { id, name, members: allMembers, messages: [] };
      await saveData();
      
      allMembers.forEach(member => {
        io.to(`user_${member}`).emit("group_chat_created", groups[id]);
      });
    });

    socket.on("send_group_message", async ({ groupId, from, text }: { groupId: string, from: string, text: string }) => {
      if (isBanned(from)) return;
      ...
      group.members.forEach(member => {
        io.to(`user_${member}`).emit("new_group_message", { groupId, message: msg });
      });
    });

    socket.on("get_group_chats", (username: string) => {
      const userGroups = Object.values(groups).filter(g => g.members.includes(username));
      socket.emit("init_group_chats", userGroups);
    });

    socket.on("register_user", async (profile: UserProfile) => {
      socketToUser.set(socket.id, profile.username);
      onlineUsers.add(profile.username);
      io.emit("online_users", Array.from(onlineUsers));
      
      // Auto-friend devs with each other
      if (DEVS.includes(profile.username)) {
        DEVS.forEach(dev => {
          if (dev !== profile.username) {
            const exists = requests.find(r => 
              (r.from === profile.username && r.to === dev) ||
              (r.from === dev && r.to === profile.username)
            );
            if (!exists) {
              requests.push({ from: dev, to: profile.username, status: 'accepted' });
              console.log(`Auto-friended ${profile.username} with ${dev}`);
            } else if (exists.status !== 'accepted') {
              exists.status = 'accepted';
              console.log(`Updated auto-friend status for ${profile.username} and ${dev}`);
            }
            
            // Explicitly notify the current user about the accepted friend
            io.to(`user_${profile.username}`).emit("friend_request_accepted", dev);
            // Also notify the other dev if they are online
            io.to(`user_${dev}`).emit("friend_request_accepted", profile.username);
          }
        });
      }

      // Send friend requests for this specific user
      const userRequests = requests.filter(r => r.from === profile.username || r.to === profile.username);
      socket.emit("init_friend_requests", userRequests);
    ...
    socket.on("get_all_users", () => {
      // Ensure devs are always in the list if they exist
      socket.emit("all_users", Object.values(users).map(({password, ...u}) => u));
    });
    ...
    socket.on("search_users", (query: string) => {
      ...
      socket.emit("search_results", results.map(({password, ...u}) => u));
    });
    ...
    socket.on("send_friend_request", async (request: FriendRequest) => {
      ...
      if (!exists) {
        // Auto-accept ONLY if BOTH are devs
        if (DEVS.includes(request.from) && DEVS.includes(request.to)) {
        ...
        if (request.status === 'accepted') {
          io.to(`user_${request.from}`).emit("friend_request_accepted", request.to);
          io.to(`user_${request.to}`).emit("friend_request_accepted", request.from);
        } else {
    ...
    socket.on("cancel_friend_request", async ({ from, to }: { from: string, to: string }) => {
      requests = requests.filter(r => !(r.from === from && r.to === to));
      await saveData();
      io.to(`user_${to}`).emit("friend_request_cancelled", { from, to });
      socket.emit("friend_request_cancelled", { from, to });
    });
    ...
    socket.on("respond_friend_request", async ({ from, to, status }: { from: string, to: string, status: 'accepted' | 'rejected' }) => {
      ...
      if (reqIdx !== -1) {
        if (status === 'rejected') {
          ...
          await saveData();
          io.to(`user_${from}`).emit("friend_request_cancelled", { from, to }); // Reuse cancel event to clear UI
        } else {
    ...
    socket.on("remove_friend", async ({ from, to }: { from: string, to: string }) => {
      requests = requests.filter(r => 
        !((r.from === from && r.to === to) || (r.from === to && r.to === from))
      );
      await saveData();
      io.to(`user_${from}`).emit("friend_removed", to);
      io.to(`user_${to}`).emit("friend_removed", from);
    });

    socket.on("ban_user", async ({ dev, target, durationHours }: { dev: string, target: string, durationHours: number }) => {
      if (!DEVS.includes(dev)) return;
      if (DEVS.includes(target)) return;
      
      const durationMs = durationHours * 60 * 60 * 1000;
      bannedUsers[target] = Date.now() + durationMs;
      await saveData();
      
      io.emit("user_banned", { target, expiration: bannedUsers[target] });
      io.to(`user_${target}`).emit("banned_notice", `You have been banned by a developer for ${durationHours} hours.`);
      
      console.log(`User ${target} banned by ${dev} for ${durationHours} hours`);
    });
