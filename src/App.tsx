...
import { AppStatus, type QuizState, type Difficulty, type Question, type CharacterDesign, type PrivateMessage, type FriendRequest, type GroupChat, type GroupMessage } from '@/types';
...
const App: FC = () => {
  ...
  const [statusAvatarRef, setStatusAvatarRef] = useState<HTMLInputElement | null>(null);
  ...
  const [profileTab, setProfileTab] = useState<'profile' | 'social'>('profile');
  ...
  const [isStudyHallOpen, setIsStudyHallOpen] = useState(false);
  const [activeAITutor, setActiveAITutor] = useState<string | null>(null);
  const [onlineUsernames, setOnlineUsernames] = useState<string[]>([]);
  const [viewingUser, setViewingUser] = useState<any | null>(null);
  const [coopRoomId, setCoopRoomId] = useState<string | null>(null);
  const [coopPlayers, setCoopPlayers] = useState<string[]>([]);
  const [isCoopHost, setIsCoopHost] = useState(false);
  const [coopJoinInput, setCoopJoinInput] = useState('');
  const coopRoomIdRef = useRef<string | null>(null);
  useEffect(() => {
    coopRoomIdRef.current = coopRoomId;
  }, [coopRoomId]);
  ...
  useEffect(() => {
    ...
    socketRef.current.on('new_friend_request', (request: FriendRequest) => {
      setFriendRequests(prev => {
        const exists = prev.find(r => r.from === request.from && r.to === request.to);
        if (exists) return prev;
        return [...prev, request];
      });
    });

    socketRef.current.on('init_friend_requests', (requests: FriendRequest[]) => {
      setFriendRequests(requests.filter(r => r.status === 'pending'));
      // Also extract friends from accepted requests
      const accepted = requests.filter(r => r.status === 'accepted');
      const friendList: string[] = [];
      accepted.forEach(r => {
        if (r.from === currentUserRef.current) friendList.push(r.to);
        else if (r.to === currentUserRef.current) friendList.push(r.from);
      });
      setFriends([...new Set(friendList)]);
    });

    socketRef.current.on('friend_request_cancelled', ({ from, to }: { from: string, to: string }) => {
      setFriendRequests(prev => prev.filter(r => !(r.from === from && r.to === to)));
    });

    socketRef.current.on('friend_request_accepted', (friend: string) => {
      setFriends(prev => [...new Set([...prev, friend])]);
      setFriendRequests(prev => prev.filter(r => r.from !== friend && r.to !== friend));
    });

    socketRef.current.on('friend_removed', (friend: string) => {
    ...
    socketRef.current.on('group_chat_created', (group: GroupChat) => {
      setGroupChats(prev => {
    ...
    socketRef.current.on('init_group_chats', (groups: GroupChat[]) => {
      setGroupChats(groups);
    });

    socketRef.current.on('new_group_message', ({ groupId, message }: { groupId: string, message: GroupMessage }) => {
      setGroupChats(prev => prev.map(g => {
        if (g.id === groupId) {
          if (g.messages.find(m => m.id === message.id)) return g;
    ...
    socketRef.current.on('coop_room_created', (roomId: string) => {
      setCoopRoomId(roomId);
      setCoopPlayers([currentUserRef.current!]);
      setIsCoopHost(true);
    });

    socketRef.current.on('coop_room_joined', ({ roomId, players }: { roomId: string, players: string[] }) => {
      setCoopRoomId(roomId);
      setCoopPlayers(players);
    });

    socketRef.current.on('coop_state_updated', (state: any) => {
      if (state.status) setStatus(state.status);
      if (state.activeQuestions) setActiveQuestions(state.activeQuestions);
      if (state.currentQuestionIndex !== undefined) setState(prev => ({ ...prev, currentQuestionIndex: state.currentQuestionIndex }));
      if (state.score !== undefined) setState(prev => ({ ...prev, score: state.score }));
      if (state.power !== undefined) setPower(state.power);
      if (state.threatLevel !== undefined) setThreatLevel(state.threatLevel);
    });

    socketRef.current.on('coop_error', (msg: string) => {
      alert(msg);
    });

    socketRef.current.on('coop_player_left', (username: string) => {
      setCoopPlayers(prev => prev.filter(p => p !== username));
      alert(`${username} has left the co-op shift.`);
    });
  ...
  useEffect(() => {
    if (currentUser && socketRef.current) {
      socketRef.current.emit('get_group_chats', currentUser);
    }
  }, [currentUser]);
  ...
  const handleSendFriendRequest = (to: string) => {
    if (!currentUser || !socketRef.current) return;
    if (friends.includes(to)) return;
    if (friendRequests.find(r => (r.from === currentUser && r.to === to) || (r.from === to && r.to === currentUser))) return;
    
    const newRequest: FriendRequest = { from: currentUser, to, status: 'pending' };
    setFriendRequests(prev => [...prev, newRequest]);
    socketRef.current.emit('send_friend_request', newRequest);
  };

  const handleCancelFriendRequest = (to: string) => {
    if (!currentUser || !socketRef.current) return;
    setFriendRequests(prev => prev.filter(r => !(r.from === currentUser && r.to === to)));
    socketRef.current.emit('cancel_friend_request', { from: currentUser, to });
  };

  const handleRespondFriendRequest = (from: string, status: 'accepted' | 'rejected') => {
    if (!currentUser || !socketRef.current) return;
    setFriendRequests(prev => prev.filter(r => !(r.from === from && r.to === currentUser)));
    if (status === 'accepted') {
  ...
  const handleBanUser = (target: string) => {
    if (!currentUser || !socketRef.current) return;
    socketRef.current.emit('ban_user', { dev: currentUser, target, durationHours: banDuration });
  };

  const handleRemoveFriend = (to: string) => {
    if (!currentUser || !socketRef.current) return;
    if (window.confirm(`Are you sure you want to remove ${to} from your friends?`)) {
      socketRef.current.emit('remove_friend', { from: currentUser, to });
    }
  };
  ...
  const handleCreateGroupChat = () => {
    if (!currentUser || !socketRef.current || !newGroupName.trim() || selectedGroupMembers.length === 0) return;
    socketRef.current.emit('create_group_chat', { 
      name: newGroupName, 
  ...
  const handleSendGroupMessage = () => {
    if (!newPrivateMessage.trim() || !currentUser || !activeGroupChat || !socketRef.current) return;
  ...
  const handleCreateCoopRoom = () => {
    if (!currentUser || !socketRef.current) return;
    socketRef.current.emit('create_coop_room', currentUser);
  };

  const handleJoinCoopRoom = () => {
    if (!currentUser || !socketRef.current || !coopJoinInput.trim()) return;
    socketRef.current.emit('join_coop_room', { roomId: coopJoinInput.trim(), username: currentUser });
    setCoopJoinInput('');
  };

  const handleLeaveCoopRoom = () => {
    if (!currentUser || !socketRef.current || !coopRoomId) return;
    socketRef.current.emit('leave_coop_room', { roomId: coopRoomId, username: currentUser });
    setCoopRoomId(null);
    setCoopPlayers([]);
    setIsCoopHost(false);
  };
  ...
  useEffect(() => {
    if (coopRoomIdRef.current && (status === AppStatus.QUIZ || status === AppStatus.ENDLESS)) {
      socketRef.current?.emit('sync_coop_state', {
        roomId: coopRoomIdRef.current,
        state: { power, threatLevel }
      });
    }
  }, [power, threatLevel, status]);
  ...
  const startNewNight = useCallback((night: number, isEndless: boolean = false) => {
    ...
    if (coopRoomIdRef.current) {
      socketRef.current?.emit('sync_coop_state', {
        roomId: coopRoomIdRef.current,
        state: {
          activeQuestions: nightQuestions,
    ...
    setTimeout(() => {
      const finalStatus = isEndless ? AppStatus.ENDLESS : AppStatus.QUIZ;
  ...
  const handleAnswer = useCallback((isCorrect: boolean) => {
    ...
    if (!isEndless && nextIndex >= activeQuestions.length) {
      const isEndingNight = gameVersion === 2 
        ? (currentNight === 3 || currentNight === 7) 
        : (currentNight === 5 || currentNight === 6 || currentNight === 7);
      
      const finalStatus = isEndingNight ? AppStatus.ENDING_CUTSCENE : AppStatus.RESULTS;
      setStatus(finalStatus);
      if (coopRoomIdRef.current) {
        socketRef.current?.emit('sync_coop_state', {
          roomId: coopRoomIdRef.current,
      ...
      const newIndex = isEndless ? (nextIndex % activeQuestions.length) : nextIndex;
      ...
      if (coopRoomIdRef.current) {
        socketRef.current?.emit('sync_coop_state', {
          roomId: coopRoomIdRef.current,
          state: { currentQuestionIndex: newIndex, score: newScore }
        });
      }
    }
  ...
  return (
    <div className={`min-h-screen w-full relative bg-black text-white ${themeClass} overflow-hidden font-['Orbitron']`}>
        ...
        <div className="flex flex-col items-center justify-center h-screen p-4 text-center space-y-6 animate-in fade-in zoom-in duration-1000 relative overflow-hidden">
            <div className="space-y-2 relative z-10">
              ...
              <div className="flex items-center justify-center gap-2 mt-2">
                 ...
                 <span className="text-[7px] text-zinc-600 font-bold uppercase tracking-widest">Academy Personnel Connected Globally</span>
              </div>
            </div>
      ...
      {status === AppStatus.START && (
        <div className="flex flex-col items-center justify-center h-screen p-4 text-center space-y-4 animate-in fade-in zoom-in duration-1000 relative overflow-hidden">
           ...
           <div className="flex flex-col md:flex-row gap-3 relative z-10">
             ...
             <button 
               onClick={() => setIsStudyHallOpen(true)} 
               className="px-8 py-3 border-2 border-zinc-800 text-zinc-500 font-black text-lg uppercase tracking-[0.2em] hover:border-blue-500 hover:text-blue-500 transition-all transform hover:scale-105 active:scale-95"
             >
               Study Hall
             </button>
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 {gameVersion === 2 ? (
                   <>
                     <button 
                       onClick={() => {
                         setPreviewConfig({ night: 3, isEnding: true });
                         setIsPreviewMode(true);
                         setIsCutscenePreviewerOpen(false);
              ...
              <div className="flex-1 flex overflow-hidden">
                 ...
                 <div className="w-64 border-r border-zinc-900 flex flex-col bg-zinc-950/50">
                    <div className="p-4 space-y-6">
                       ...
                       <div className="space-y-2">
                          <h3 className="text-[10px] text-zinc-600 font-black uppercase tracking-widest">Direct_Messages</h3>
                          {friends.length === 0 ? (
                            ...
                            friends.map(f => (
                              <div key={f} className="flex gap-1">
                                <button 
                                  ...
                                  className={`flex-1 text-left p-2 border transition-all flex items-center justify-between ${activeChat === f ? 'bg-blue-900/20 border-blue-600 text-white' : 'bg-zinc-900 border-zinc-800 text-zinc-500 hover:border-zinc-600'}`}
                                >
                                   <span className="text-[10px] font-bold uppercase">{f}</span>
                                   <div className={`w-1.5 h-1.5 rounded-full ${onlineUsernames.includes(f) ? 'bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.5)] animate-pulse' : 'bg-zinc-800'}`}></div>
                                </button>
                                <button 
                                  onClick={() => handleRemoveFriend(f)}
                                  className="px-2 bg-zinc-900 border border-zinc-800 text-zinc-700 hover:text-red-500 hover:border-red-900 transition-all"
                                  title="Remove Friend"
                                >
                                   <i className="fa-solid fa-user-minus text-[8px]"></i>
                                </button>
                              </div>
                            ))
                          )}
                       ...
                       <div className="space-y-2 pt-4 border-t border-zinc-900">
                          <h3 className="text-[10px] text-zinc-600 font-black uppercase tracking-widest">Group_Chats</h3>
                          {!coopRoomId ? (
                            <div className="space-y-2">
                               <button 
                                 onClick={() => setIsGroupModalOpen(true)}
                                 className="w-full bg-blue-900/20 border border-blue-900/50 text-blue-500 text-[10px] py-2 uppercase font-black hover:bg-blue-900/40 transition-all"
                               >
                                 MAKE A GROUP CHAT
                               </button>
                                 ...
                                 <div className="space-y-1 max-h-32 overflow-y-auto custom-scrollbar pr-1">
                                   {groupChats.map(g => (
                                     <button 
                                       ...
                                       className={`w-full text-left p-2 border ${activeGroupChat === g.id ? 'bg-blue-900/20 border-blue-900/50 text-white' : 'bg-zinc-900/50 border-zinc-800 text-zinc-400'} hover:border-zinc-600 transition-all flex justify-between items-center`}
                                     >
                                       <span className="text-[10px] font-bold uppercase truncate pr-2">{g.name}</span>
                               ...
                               <div className="flex gap-1">
                                  <input 
                                    type="text" 
                                    value={coopJoinInput}
                                    onChange={(e) => setCoopJoinInput(e.target.value)}
                                    placeholder="GROUP ID"
                                    className="flex-1 bg-zinc-900 border border-zinc-800 text-[10px] p-1 text-white outline-none focus:border-blue-900"
                                  />
                                  <button 
                                    onClick={handleJoinCoopRoom}
                                    className="bg-zinc-800 text-zinc-400 text-[10px] px-2 uppercase font-black hover:bg-white hover:text-black transition-all"
                                  >
                                    Join
                                  </button>
                               </div>
                            ...
                            <div className="bg-zinc-900 p-2 border border-blue-900/30 space-y-2">
                               <div className="flex justify-between items-center">
                                  <span className="text-[10px] text-blue-500 font-black tracking-tighter">ROOM: {coopRoomId}</span>
                                  <button onClick={handleLeaveCoopRoom} className="text-zinc-600 hover:text-red-500"><i className="fa-solid fa-right-from-bracket text-[10px]"></i></button>
                               </div>
                               <div className="space-y-1">
                                  {coopPlayers.map(p => (
                                    <div key={p} className="flex items-center gap-2">
                                       <div className="w-1 h-1 bg-green-500 rounded-full animate-pulse"></div>
                                       <span className="text-[9px] text-zinc-400 font-bold uppercase">{p} {p === currentUser && '(YOU)'}</span>
                                    </div>
                                  ))}
                               </div>
                               {isCoopHost && coopPlayers.length >= 2 && (
                                 <button 
                                   onClick={() => {
                                     setIsSocialOpen(false);
                                     handleStartShift();
                                     socketRef.current?.emit('sync_coop_state', { roomId: coopRoomId, state: { status: AppStatus.NIGHT_CUTSCENE } });
                                   }}
                                   className="w-full bg-green-900/20 border border-green-900/50 text-green-500 text-[10px] py-2 uppercase font-black hover:bg-green-900/40 transition-all mt-2"
                                 >
                                   Start Co-op Shift
                                 </button>
                               )}
                  ...
                  <div className="flex-1 flex flex-col bg-zinc-950">
                     {!activeChat && !activeGroupChat ? (
                      <div className="flex-1 flex flex-col p-8 space-y-8 overflow-hidden">
                         <div className="space-y-4">
                            <h2 className="text-3xl font-black italic uppercase tracking-tighter text-white">Personnel_Database</h2>
                            <div className="relative">
                               <input 
                                 type="text"
                                 value={searchQuery}
                                 onChange={(e) => handleSearchUsers(e.target.value)}
                                 placeholder="Search by username (active or inactive)..."
                         ...
                         <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2">
                            <div className="flex items-center justify-between border-b border-zinc-900 pb-2">
                               <span className="text-[10px] text-zinc-600 font-black uppercase tracking-widest">
                                 {searchQuery.length > 1 ? `Search_Results: ${searchResults.filter(u => u.username !== currentUser).length}` : `All_Registered_Personnel: ${allUsers.filter(u => u.username !== currentUser).length}`}
                               </span>
                            </div>
                            
                            {(searchQuery.length > 1 ? searchResults : allUsers).filter(u => u.username !== currentUser).map(u => (
                              <div key={u.username} className="bg-zinc-900/50 border border-zinc-800 p-4 flex items-center justify-between group hover:border-blue-900 transition-all">
                                 <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex flex-col">
                                       ...
                                       {u.description && <span className="text-[8px] text-zinc-500 uppercase italic truncate max-w-[200px]">{u.description}</span>}
                                    </div>
                                 </div>
                                 <div className="flex gap-2">
                                    <button 
                                      onClick={() => setViewingUser(u)}
                                      className="px-4 py-2 bg-zinc-800 text-zinc-400 border border-zinc-700 font-bold uppercase text-[10px] hover:bg-zinc-700 hover:text-white transition-all flex items-center gap-2"
                                    >
                                      <i className="fa-solid fa-eye"></i>
                                      Show
                                    </button>
                                    {(currentUser === 'blamer_508' || currentUser === 'ellieblockman') && u.username !== 'blamer_508' && u.username !== 'ellieblockman' && (
                                      <div className="flex items-center gap-2 mr-4 border-r border-zinc-800 pr-4">
                                         <input 
                                           ...
                                           className="w-12 bg-zinc-950 border border-zinc-800 text-[10px] p-1 text-red-500 font-mono"
                                           title="Ban Duration (Hours)"
                                         />
                                         <button 
                                           onClick={() => handleBanUser(u.username)}
                                           className="px-3 py-2 bg-red-900/40 text-red-500 border border-red-900 font-black uppercase text-[10px] hover:bg-red-600 hover:text-white transition-all"
                                         >
                                           Ban
                                         </button>
                                      </div>
                                    ...
                                    {!currentUser ? (
                                      <button onClick={() => setStatus(AppStatus.LOGIN)} className="px-6 py-2 bg-zinc-800 text-zinc-500 border border-zinc-700 font-bold uppercase text-[10px] hover:bg-zinc-700 transition-all">Login to Add</button>
                                    ) : friends.includes(u.username) ? (
                                      <>
                                        <span className="px-4 py-2 bg-zinc-900 text-zinc-500 border border-zinc-800 font-bold uppercase text-[10px] flex items-center">Friend</span>
                                    ...
                                    ) : friendRequests.find(r => r.from === currentUser && r.to === u.username) ? (
                                      <button onClick={() => handleCancelFriendRequest(u.username)} className="px-6 py-2 bg-red-900/20 text-red-500 border border-red-900 font-bold uppercase text-xs hover:bg-red-900/40 transition-all">Cancel Request</button>
                                    ) : friendRequests.find(r => r.from === u.username && r.to === currentUser) ? (
                                      <div className="flex gap-1">
                                        <button onClick={() => handleRespondFriendRequest(u.username, 'accepted')} className="px-3 py-2 bg-green-900/20 text-green-500 border border-green-900 font-bold uppercase text-[10px] hover:bg-green-900/40 transition-all">Accept</button>
                                        <button onClick={() => handleRespondFriendRequest(u.username, 'rejected')} className="px-3 py-2 bg-red-900/20 text-red-500 border border-red-900 font-bold uppercase text-[10px] hover:bg-red-900/40 transition-all">Decline</button>
                                      </div>
                                    ) : (
                      ...
                      <div className="flex-1 flex flex-col overflow-hidden">
                         ...
                         <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                            {privateMessages.filter(m => (m.from === activeChat && m.to === currentUser) || (m.from === currentUser && m.to === activeChat)).length === 0 ? (
                              <p className="text-zinc-800 text-center uppercase font-bold tracking-widest mt-20 italic">No message history detected</p>
                            ) : (
                              privateMessages.filter(m => (m.from === activeChat && m.to === currentUser) || (m.from === currentUser && m.to === activeChat)).map(m => (
                      ...
                      <div className="flex-1 flex flex-col overflow-hidden">
                         <div className="p-4 border-b border-zinc-900 flex justify-between items-center bg-zinc-900/20">
                            <div className="flex items-center gap-3">
                               <button onClick={() => setActiveGroupChat(null)} className="text-zinc-600 hover:text-white"><i className="fa-solid fa-arrow-left"></i></button>
                               <span className="text-lg font-black uppercase tracking-tighter text-white">Group: {groupChats.find(g => g.id === activeGroupChat)?.name}</span>
                            </div>
                         </div>
                         <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                            {groupChats.find(g => g.id === activeGroupChat)?.messages.length === 0 ? (
                              <p className="text-zinc-800 text-center uppercase font-bold tracking-widest mt-20 italic">No group message history detected</p>
                            ) : (
                              groupChats.find(g => g.id === activeGroupChat)?.messages.map(m => (
                                <div key={m.id} className={`flex flex-col ${m.from === currentUser ? 'items-end' : 'items-start'}`}>
                                   <div className={`max-w-[80%] p-3 rounded-sm border font-mono text-[11px] ${m.from === currentUser ? 'bg-blue-900/20 border-blue-900/50 text-blue-100' : 'bg-zinc-900 border-zinc-800 text-zinc-300'}`}>
                                      {m.text}
                                   </div>
                                   <span className="text-[7px] text-zinc-700 mt-1 uppercase font-bold">{m.from} • {new Date(m.timestamp).toLocaleTimeString()}</span>
                                </div>
                              ))
                            )}
                         </div>
      ...
      <AnimatePresence>
        {viewingUser && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[10002] flex items-center justify-center p-4 bg-black/95 backdrop-blur-2xl"
          >
            <div className="w-full max-w-2xl bg-zinc-950 border-4 border-zinc-900 p-8 relative shadow-[0_0_100px_rgba(0,0,0,1)]">
              <button 
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="w-48 h-48 bg-zinc-900 border-2 border-zinc-800 rounded-sm overflow-hidden flex-shrink-0">
                  {viewingUser.avatarUrl ? (
                    <img src={viewingUser.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                  ) : (
                    <i className="fa-solid fa-user text-zinc-800 text-6xl flex items-center justify-center h-full"></i>
                  )}
                </div>
                ...
                <div className="flex-1 space-y-6">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-4xl font-black italic uppercase text-white tracking-tighter">{viewingUser.username}</h3>
                      {onlineUsernames.includes(viewingUser.username) && (
                        <span className="px-2 py-0.5 bg-green-900/40 text-green-500 border border-green-900 text-[8px] font-black uppercase tracking-widest animate-pulse">Online</span>
                      )}
                    </div>
                    <p className="text-zinc-500 text-[10px] uppercase tracking-[0.3em] font-bold">Personnel_File_ID: {Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
                  </div>

                  <div className="space-y-2">
                    <span className="text-[10px] text-zinc-600 font-black uppercase tracking-widest block">Description_Log</span>
                    <div className="bg-zinc-900/50 border border-zinc-800 p-4 min-h-[80px]">
                      <p className="text-zinc-400 text-xs italic leading-relaxed">
                        {viewingUser.description || "No personnel description provided for this account."}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <span className="text-[10px] text-zinc-600 font-black uppercase tracking-widest block">System_Clearance</span>
                      <div className="bg-zinc-900/50 border border-zinc-800 p-3">
                        <p className="text-white font-black text-xl italic uppercase">Night_{viewingUser.maxUnlockedNight || 1}</p>
                      </div>
                    </div>
                    <div className="space-y-2">
                      <span className="text-[10px] text-zinc-600 font-black uppercase tracking-widest block">Performance_Rating</span>
                      <div className="bg-zinc-900/50 border border-zinc-800 p-3">
                        <p className="text-blue-500 font-black text-xl italic uppercase">
                          {viewingUser.highScores ? Math.max(...Object.values(viewingUser.highScores as Record<string, number>)) : 0} PTS
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex gap-4">
                    {currentUser && viewingUser.username !== currentUser && !friends.includes(viewingUser.username) && (
                      <button 
                        onClick={() => { handleSendFriendRequest(viewingUser.username); setViewingUser(null); }}
                        className="flex-1 py-3 bg-blue-900/40 text-blue-500 border border-blue-900 font-black uppercase text-xs hover:bg-blue-600 hover:text-white transition-all"
                      >
                        Send_Friend_Request
                      </button>
                    )}
                    {currentUser && viewingUser.username !== currentUser && friends.includes(viewingUser.username) && (
                      <button 
                        onClick={() => { handleStartChat(viewingUser.username); setViewingUser(null); }}
                        className="flex-1 py-3 bg-green-900/40 text-green-500 border border-green-900 font-black uppercase text-xs hover:bg-green-600 hover:text-white transition-all"
                      >
                        Open_Direct_Comms
                      </button>
                    )}
        ...
        {isGroupModalOpen && (
          ...
          >
            <div className="w-full max-w-md bg-zinc-950 border-2 border-zinc-900 p-6 space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-black italic uppercase text-white tracking-tighter">Initialize_Group</h2>
                <button onClick={() => setIsGroupModalOpen(false)} className="text-zinc-600 hover:text-white"><i className="fa-solid fa-xmark"></i></button>
              </div>

              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] text-zinc-600 font-black uppercase tracking-widest">Group_Name</label>
                  <input 
                    type="text"
                    value={newGroupName}
                    onChange={(e) => setNewGroupName(e.target.value)}
                    placeholder="Enter group designation..."
                    className="w-full bg-zinc-900 border border-zinc-800 p-3 text-white font-mono text-xs outline-none focus:border-blue-900"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-[10px] text-zinc-600 font-black uppercase tracking-widest">Select_Personnel ({selectedGroupMembers.length})</label>
                  <div className="max-h-48 overflow-y-auto custom-scrollbar space-y-1 pr-2">
                    {friends.length === 0 ? (
                      <p className="text-[10px] text-zinc-800 italic uppercase font-bold">No verified friends detected</p>
                    ) : (
                      friends.map(friend => (
                        <button
                          key={friend}
                          ...
                          className={`w-full text-left p-2 border text-[10px] font-bold uppercase transition-all flex justify-between items-center ${
                            selectedGroupMembers.includes(friend) 
                              ? 'bg-blue-900/20 border-blue-900/50 text-white' 
                              : 'bg-zinc-900/50 border-zinc-800 text-zinc-500'
                          }`}
                        >
                          <span>{friend}</span>
                          {selectedGroupMembers.includes(friend) && <i className="fa-solid fa-check text-blue-500"></i>}
                        </button>
                      ))
                    )}
                  </div>
              ...
              <button 
                ...
                className="w-full bg-blue-900 hover:bg-blue-600 disabled:bg-zinc-900 disabled:text-zinc-700 text-white py-4 font-black uppercase tracking-widest transition-all"
              >
                Create Group
              </button>
            </div>
          ...
          >
            <div className="w-full max-w-4xl space-y-8">
              <div className="text-center space-y-2">
                <h2 className="text-5xl font-black italic uppercase tracking-tighter text-white">Study Hall</h2>
                <p className="text-zinc-500 uppercase font-bold tracking-[0.3em] text-xs">Select a Subject Expert AI</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { id: 'Computer Science', icon: 'fa-microchip', color: 'text-blue-500', bg: 'bg-blue-900/10', border: 'border-blue-900/30' },
                  { id: 'Software Engineering', icon: 'fa-code', color: 'text-green-500', bg: 'bg-green-900/10', border: 'border-green-900/30' },
                  { id: 'Natural Sciences', icon: 'fa-flask', color: 'text-purple-500', bg: 'bg-purple-900/10', border: 'border-purple-900/30' },
                  { id: 'Mathematics', icon: 'fa-calculator', color: 'text-amber-500', bg: 'bg-amber-900/10', border: 'border-amber-900/30' },
                  { id: 'Friendly AI', icon: 'fa-heart', color: 'text-pink-500', bg: 'bg-pink-900/10', border: 'border-pink-900/30' }
                ].map(subject => (
                  <button
                    key={subject.id}
                    onClick={() => { setActiveAITutor(subject.id); setIsStudyHallOpen(false); }}
                    className={`p-6 ${subject.bg} border ${subject.border} hover:border-white transition-all group text-left space-y-4`}
                  >
                    <div className={`w-12 h-12 rounded-sm bg-zinc-950 border ${subject.border} flex items-center justify-center group-hover:scale-110 transition-transform`}>
                      <i className={`fa-solid ${subject.icon} ${subject.color} text-xl`}></i>
                    </div>
                    <div>
                      <h3 className="text-white font-black uppercase tracking-tighter text-lg">{subject.id}</h3>
                      <p className="text-zinc-500 text-[10px] uppercase font-bold tracking-widest leading-tight">Consult with our specialized AI expert.</p>
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="flex justify-center pt-8">
                <button 
                  onClick={() => setIsStudyHallOpen(false)}
                  className="px-12 py-3 bg-zinc-900 text-zinc-500 font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all"
                >
                  Exit Study Hall
                </button>
              </div>
      ...
      <AnimatePresence>
        {activeAITutor && (
          <AITutor 
            subject={activeAITutor} 
            onClose={() => setActiveAITutor(null)} 
          />
        )}
      </AnimatePresence>
      <style>{`
